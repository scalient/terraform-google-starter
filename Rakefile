...
project_base_dir = Pathname.new("modules/project_base")
...
Utilities.create_terraform_tasks(
  ...
  Open3.popen3("terraform", "output", "-json") do |stdin, stdout, _stderr, wait_thr|
    stdin.close
  ...
  Open3.popen3("terraform", "console", "-var-file", "terraform.tfvars") do |stdin, stdout, _stderr, wait_thr|
    # Avoid deadlock.
    Thread.new do
      heredoc_content = <<EOS
...
Utilities.create_terraform_tasks(
  ...
  [
    ...
    prerequisites_dir, project_base_dir, user_tfvars, *current_dir.glob("*.tf"),
  ],
) do
  sh "terraform", "apply"
end
...
Utilities.create_terraform_tasks(
  ...
  [
    receipts["main"],
    prerequisites_dir, project_base_dir, kubernetes_terraform_dir, user_tfvars,
  ],
) do
  bucket = stage_bootstrap_tfvars.open("rb") do |f|
    JSON.parse(f.read)["remote_state_bucket"]
  ...
  sh "git", "add", "-f", "--", kubernetes_terraform_dir.join("providers.tf").to_s
end
